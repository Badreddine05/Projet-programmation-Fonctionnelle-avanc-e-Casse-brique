        ...
        (fun j (e2 : t) ->
          ...
          if j > i && (Float.is_finite m1 || Float.is_finite m2) then begin
              ...
              then begin
                  e1#touched#set true;
                  e2#touched#set true;
                  (* Si on touche une des raquettes gauche ou droite, la direction est déterminée, on garde la même vitesse et on se dirige dans la direction d'un vecteur fixé *)
                  match (e1#nature#get, e2#nature#get) with
                  | (Type.Player (Type.Left), Type.Ball) ->
                     (let n = Vector.norm e2#velocity#get in
                      e2#velocity#set (Vector.mult n Vector.{x = -0.6; y = -0.8;})
                     )
                  | (Type.Ball, Type.Player (Type.Left)) -> 
                     (let n = Vector.norm e1#velocity#get in
                      e1#velocity#set (Vector.mult n Vector.{x = -0.6; y = -0.8;})
                     )
                  | (Type.Player (Type.Right), Type.Ball) ->
                     (let n = Vector.norm e2#velocity#get in
                      e2#velocity#set (Vector.mult n Vector.{x = 0.6; y = -0.8;})
                     )
                  | (Type.Ball, Type.Player (Type.Right)) -> 
                     (let n = Vector.norm e1#velocity#get in
                      e1#velocity#set (Vector.mult n Vector.{x = 0.6; y = -0.8;})
                     )
                  | (_,_) ->
              ...
              let e = match (e1#nature#get, e2#nature#get) with
                | (_,Type.Block) -> e2#resistance#get
                | (Type.Block,_) -> e1#resistance#get
                | (_,_) -> 1.0
              in
